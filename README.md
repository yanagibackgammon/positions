...
- `Double`だけではなく、相手の最適応手まで含めます。
- 最善アクションを先頭にし、最大3候補を表示します。
- 2番手以降には、最善アクションとのEquity差を併記します。
...
- 最善アクションは`Double/Take`または`Double/Pass`です。
- 候補は最大2つです。
...
Checker Playでは、実戦で選択した候補手の解析値を表示します。Cube Actionでも、実際に選択されたアクションから取得できる解析値を使用します。Passなど確率ベクトルを取得できないアクションは`—`と表示します。
